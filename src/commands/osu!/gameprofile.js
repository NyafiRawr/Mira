...
module.exports = {
  ...
  async execute(message, args /* , CooldownReset */) {
    ...
    const osuUser = await osu.getUser(player.gameServer, player.nickname, player.modes);
    ...
    if (osuUser === null) {
      return message.reply(`игрок __**${player.nickname}**__ не найден на сервере __**${server}**__ (режим: __**${player.mode}**__).`);
    }

    const serverLinks = tools.getDataValueOnKey('osu!/links', player.gameServer);

    const embed = new Discord.RichEmbed()
      .setAuthor(`osu! ${mode} на ${server}`, serverLinks.flag.replace('AZ', osuUser.country))
      .setTitle(osuUser.username)
      .setURL(serverLinks.user.replace('ID', osuUser.user_id).replace('MODE', dataMode.mode))
      .setDescription('**Место в мире:** '
        + `[#${tools.separateThousandth(osuUser.pp_rank)}](${serverLinks.pp_world
          .replace('MODE', dataMode.mode)
          .replace('AZ', osuUser.country)
          .replace('P', Math.ceil(osuUser.pp_rank / 50))}) `
        + `(**${osuUser.country}**[#${tools.separateThousandth(osuUser.pp_country_rank)}](${serverLinks.pp_country
          .replace('MODE', dataMode.mode)
          .replace('AZ', osuUser.country)
