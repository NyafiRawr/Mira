  ...
  async execute(message /* , args , CooldownReset */) {
    ...
    switch (change) {
      case '0': {
        ...
        embedMessage = await embedMessage.edit(message.author, { embed });
        const osuModesIndexes = await menu.waitReactionComplete(embedMessage,
          menu.emojiNumbers.slice(1, Object.keys(modes).length + 1), menu.emojiComplete,
          message.author.id);
      ...
      case '1': {
        ...
        const listServersPlayer = await players.get(message.author.id);
        ...
        for (let i = 0; i < listServersPlayer.length; i += 1) {
          changeServer += `${menu.emojiNumbers[i + 1]} ${servers[listServersPlayer[i].gameServer].name}\n`;
        }
        embed.setDescription(changeServer);
        embedMessage = await embedMessage.edit(message.author, { embed });
        const osuServerIndex = await menu.waitReaction(embedMessage,
          menu.emojiNumbers.slice(1, listServersPlayer.length + 1), message.author.id);
        await embedMessage.clearReactions();
        await players.remove(message.author.id, listServersPlayer[osuServerIndex].gameServer)
          .then(() => {
            embed.setTitle('Удалено');
            embed.setDescription(`Аккаунт **${listServersPlayer[osuServerIndex].nickname}** при сервере **${servers[listServersPlayer[osuServerIndex].gameServer].name}** отвязан`);
