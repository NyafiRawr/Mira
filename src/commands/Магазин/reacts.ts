...
module.exports = {
  ...
  async execute(message: Discord.Message, args: string[]) {
    ...
    if (args.length === 0 || !isNaN(parseInt(args[0], 10))) {
      const rrOnServer = await emojis.getServer(message.guild.id);
      if (!rrOnServer?.length) {
        throw new CustomError(`ничего, нигде, никогда не выдаётся. Начать выдавать: \`${this.aliases[0]} ${this.usage}\``);
      }
      ...
      if (args.length !== 0) {
        for (const num of [...new Set(args)]) {
          ...
          await emojis.remove(rrOnServer[n].serverId, rrOnServer[n].channelId, rrOnServer[n].messageId, rrOnServer[n].emojiId);
          message.reply({
            'embed': {
              'title': `${this.description} - удалено!`,
              'description': `${n + 1}. <#${rrOnServer[n].channelId}> [№${rrOnServer[n].messageId}](https://discordapp.com/channels/${rrOnServer[n].serverId}/${rrOnServer[n].channelId}/${rrOnServer[n].messageId}):\n`
                + `<@&${rrOnServer[n].roleId}> - ${isNaN(parseInt(rrOnServer[n].emojiId, 10)) ? rrOnServer[n].emojiId : message.guild.emojis.get(rrOnServer[n].emojiId)?.name}`,
              'color': oneColor
            }
          });
        }
      ...
      for (let i = 0; i < rrOnServer.length; i += 1) {
        message.channel.send({
          'embed': {
            'description': (
              `${i + 1}. <#${rrOnServer[i].channelId}> [№${rrOnServer[i].messageId}](https://discordapp.com/channels/${rrOnServer[i].serverId}/${rrOnServer[i].channelId}/${rrOnServer[i].messageId}):\n`
              + `<@&${rrOnServer[i].roleId}> - ${isNaN(parseInt(rrOnServer[i].emojiId, 10)) ? rrOnServer[i].emojiId : message.guild.emojis.get(rrOnServer[i].emojiId)?.name}`).slice(0, 1300),
            'color': oneColor
          }
        });
      }
    ...
    if (!message.member.hasPermission(this.permissions[1])) {
      throw new CustomError(
        `мне нужно право добавлять реакции в ${channel}!`
      );
    }
    ...
    for (let i = 0; i < message.mentions.roles.size; i += 1) {
      ...
      if (!reaction.startsWith('<')) {
        if (await messageFetch.react(reaction).catch(() => null) === null) continue;
        await emojis.set(message.guild.id, channel.id, messageId, reaction, role.id);
      } else {
        const reactionId = reaction.slice(
          reaction.lastIndexOf(':') + 1,
          reaction.length - 1
        );
        if (await messageFetch.react(reactionId).catch(() => null) === null) continue;
        await emojis.set(message.guild.id, channel.id, messageId, reactionId, role.id);
      }
    }
