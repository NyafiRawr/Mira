        ...
        if (args.shift() == 'edit') {
            ...
            switch (element) {
                case 'desc': {
                    if (
                        user.reputation <
                        config.socialProfiles.reputationEditDescription
                    ) {
                        throw new Error(
                    ...
                    if (source.length === 0) {
                        user = await user.update({
                            biographyDescription: null,
                        });
                    } else {
                        user = await user.update({
                            biographyDescription: source,
                        });
                    }
                    break;
                }
                case 'img': {
                    if (
                        user.reputation <
                        config.socialProfiles.reputationEditImage
                    ) {
                        throw new Error(
                ...
                case 'color': {
                    if (
                        user.reputation <
                        config.socialProfiles.reputationEditColorLine
                    ) {
                        throw new Error(
                    ...
                    if (source.length === 0) {
                        ...
                        if (config.patternHexColor.test(source) == false) {
                            throw new Error(
                                'цвет должен быть указан в формате #HEX.'
                            );
                        }
                        user = await user.update({
                            biographyLineColor: source,
                        });
                    }
                    break;
                }
