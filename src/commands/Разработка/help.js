  ...
  execute(message, args /* , CooldownReset */) {
    ...
    if (!args.length) {
      ...
      for (const g of groups) {
        const catList = [...new Set(commands.filter((cmd) => (cmd.hide !== true) && (cmd.group === g) && (cmd.permissions === undefined)).map((command) => `${config.bot.prefix}**${command.name}** ${command.usage || ''} - ${command.description}`))].join('\n').substring(0, 1024);
        if (catList) {
          embed.addField(g, catList, false);
        }
      }
      embed.addField('Подробная информация о команде', `${config.bot.prefix}**help** [имя команды]`, false);
    } else {
      ...
      if (command.usage) {
        embed.addField('**Использовать так**', `${config.bot.prefix}${command.name} ${command.usage}`, false);
      }
      ...
      if (command.permissions) {
        embed.addField('**Требуемые права**', command.permissions.join(', '), true);
      }
    }
