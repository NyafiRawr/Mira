        ...
        if (args.length == 0) {
            ...
            if (args[0] == 'add') {
                ...
                if (!message.mentions.roles.size) {
                    throw new Error(
                        'не упомянута роль для выставления на продажу.'
                    );
                }
                ...
                embed.setDescription(
                    `**${role}** выставлена за **${cost}**:cookie:`
                );
            } else if (args[0] == 'rem') {
                if (!message.member?.hasPermission(this.permissions[0])) {
                    throw new Error(
                ...
                const isRemove = message.member!.roles.cache.has(
                    roles[num - 1].roleId
                );
                ...
                if (isRemove === false) {
                    ...
                    if (haveShopRole) {
                        throw new Error(
                            'можно иметь только одну роль из магазина.'
                        );
                    }
                }
                ...
                if (isRemove) {
                    const partOfFunds = Math.round(
                        (roles[num - 1].cost * 70) / 100
                    ); // 70%
                    ...
                    embed
                        .setDescription(
                            `Роль <@&${roles[num - 1].roleId}> продана!`
                        )
                        .setFooter(
                    ...
                    embed.setDescription(
                        `Роль <@&${roles[num - 1].roleId}> на тебе!`
                    );
                }
            }
        }
