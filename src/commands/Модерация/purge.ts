...
module.exports = {
    ...
    async execute(message: Message, args: string[]) {
        ...
        if (
            !channel?.permissionsFor(message.member!)?.has(this.permissions[0])
        ) {
            throw new Error(
                'у тебя нет права управлять сообщениями в этом канале!'
            );
        }
        if (
            !channel
                ?.permissionsFor(message.client.user!)!
                .has(this.permissions[0])
        ) {
            throw new Error(
        ...
        if (isInteger(amount) == false) {
            throw new Error(
                'количество должно быть целочисленным и положительным.'
            );
        }
